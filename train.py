...
train_datasets = MyDatasets(train_directory,"./datasets/train/train_label.txt", transform=train_transforms)
...
valid_datasets = MyDatasets(valid_directory,"./datasets/val/val_label.txt",transform=test_valid_transforms)
...
resnet50 = ActionDetect_Net().cuda()

#loss_func = nn.MesLoss()
...
def train_and_valid(model, loss_function, optimizer, epochs=25):
    ...
    for epoch in range(epochs):#训练epochs轮
        ...
        valid_loss = 0.0
        valid_acc = 0.0
        for i, (inputs,labels,pos) in enumerate(train_data):
        ...
        with torch.no_grad():
            model.eval()#验证
            for j, (inputs, labels,pos) in enumerate(valid_data):
